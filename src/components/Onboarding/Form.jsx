...
function Form({id, label, type, src, placeholder, value, handleChange, error}) {
  ...
  return (
    <div className= {`w-full ${error ? "mb-4" : "mb-1"}`}>
        <label htmlFor={id} className="satoshi font-bold align-middle text-[12px] leading-[18px]">{label}</label>
        <div className="relative flex items-center mt-1 ">
            <img src={src} alt={`${src}-image`} className="absolute h-6 rounded-[6px] left-1.5 bg-white z-10"/>
            {id === "password" && <img src={open ? 'eye-slash.svg' : "open-eye.svg"} onClick={()=>setOpen(!open)} className="absolute right-6 z-10 cursor-pointer"/> }
