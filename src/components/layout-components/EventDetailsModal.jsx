...
const EventDetailsModal = ({ isOpen, onClose, event }) => {
  ...
  const goingGuests =
    event.going?.filter(guest => guest.status === "going") || [];
      ...
      >
        ...
        {viewState !== 3 && (
          <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
            ...
            <div
              className={`w-fit flex flex-col py-8 ${viewState === 2 ? "px-10 bg-white" : "px-8"} gap-4 flex-shrink-0 overflow-y-auto scrollbar-hide`}
            >
              {viewState === 2 && (
                <div className="">
                  <TagButton
              ...
              <div className="grid gap-4">
                <div
                  className={`${viewState === 2 ? "size-[393px]" : "size-[349px]"} overflow-hidden rounded-3xl border-4 border-white flex-shrink-0`}
                >
                  <img
                    src={event.image}
                    alt={event.title}
              ...
              <div className="grid gap-1">
                ...
                <div className="flex gap-1 items-center justify-between">
                  <div className="flex gap-1 items-center">
                    <Avatar
                      size="xs"
                      src={event.host?.photo?.url ? event.host.photo.url : ""}
                    />
                    <div className="grid">
            ...
            <div
              className={`w-full min-w-0 flex flex-col h-full bg-white/80 relative border-l border-[#E5E7E3]  ${viewState === 2 && "rounded-tl-2xl"}`}
            >
              <section
                className={`sticky top-0 right-0 z-10 flex gap-6 flex-col py-6 px-8 border-b border-[#E5E7E3]`}
              >
                <div className={`flex gap-2 justify-between`}>
                  ...
                  <div className="flex gap-4">
                    ...
                    <TextButton
                      ...
                      rightImg={
                        viewState === 1 ? (
                          <Maximize1 variant="Bulk" />
                        ) : (
                          <SidebarRight variant="Bulk" />
                        )
                      }
                ...
                <h2
                  id="modal-title"
                  className="text-3xl font-normal paytone text-[#001010]"
                >
                  {event.title}
                </h2>
              </section>
              ...
              <div className="overflow-y-auto flex flex-col scrollbar-hide border-b border-[#E5E7E3]">
                  ...
                  <div className="flex overflow-x-auto scrollbar-hide gap-4 p-4">
                    <div className="flex overflow-x-auto scrollbar-hide gap-4 p-4">
                      {event.going.map((guest, index) => (
                        <div
                          key={index}
                          className="flex flex-col items-center justify-center gap-1 w-[112px] shrink-0 h-[90px] p-3 rounded-[12px] bg-[#FFFFFF] drop-shadow-[0px_4px_16px_rgba(0,0,0,0.04)]"
                        >
                ...
                <div
                  className={`mt-auto flex flex-col justify-end sm:flex-row gap-3 px-8 pb-8 sticky bottom-0 bg-gradient-to-b from-[#e8e8e8]/0 to-[#FFFFFF] z-10 pt-6`}
                >
                  <ConfirmationButton
                    variant="going"
                    className={`${viewState === 1 ? "max-w-[222px]" : "max-w-[300px]"}`}
                  />
                  <ConfirmationButton
                    variant="not-sure"
                    className={`${viewState === 1 ? "max-w-[222px]" : "max-w-[300px]"}`}
              ...
              <section className="w-[513px] h-fit flex flex-col gap-6 place-items-center">
                <div className="grid gap-4 place-items-center">
                  <div className="grid text-center place-items-center">
                    ...
                    <h1 className="paytone text-2xl font-normal">
                      {event.title}
                    </h1>
                  </div>
                  <EventStatus title="draft" size="sm" color="bluegreen" />
                ...
                <div className="grid gap-2 place-items-center">
                  <div className="size-fit flex gap-2 items-center">
                    ...
                    <h6 className="text-base font-medium text-black">
                      {event.date}{" "}
                      <span className="text-[#8A9191] ml-2">{event.time}</span>
                    </h6>
                  </div>
                  <div className="size-fit flex gap-2 items-center">
                    <TagButton
                    ...
                    />
                    <TagButton
                      text={`From - ₦${event.chipInDetails?.amount}`}
                      variant="light-purple"
