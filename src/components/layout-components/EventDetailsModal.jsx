...
const EventDetailsModal = ({ isOpen, onClose, event }) => {
  ...
  // 1. Handle Escape Key (Safe to re-run if onClose changes)
  useEffect(() => {
    if (!isOpen) return;
  ...
  // 2. Handle Body Scroll Lock (Only depends on isOpen)
  useEffect(() => {
    if (isOpen) {
  ...
  // 3. Handle Initial Focus (Only runs when opened)
  useEffect(() => {
    if (isOpen) {
  ...
  const descriptionLimit = viewState === 2 ? Math.round(BASE_DESCRIPTION_LIMIT * 2) : BASE_DESCRIPTION_LIMIT;
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={`relative overflow-hidden flex flex-col transition-all duration-300 ease-in-out focus:outline-none shadow-2xl ${
          viewState === 1
            ? "w-[1056px] h-[670px] rounded-3xl bg-[#E6E5E5]"
            : viewState === 2
              ? "w-screen h-screen rounded-none bg-white"
              : "w-screen h-screen rounded-none bg-[#F0F0F0]"
        }`}
      >
        {/* The unified Timer Navbar */}
        ...
        {viewState !== 3 && (
          <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
            ...
            {/* --- Right Side: Event Details & Action Buttons --- */}
              ...
              <section className="sticky top-0 right-0 z-10 flex gap-6 flex-col py-6 px-8 border-b border-[#E5E7E3]">
                <div className="flex gap-2 justify-between">
                  ...
                  <div className="flex gap-4">
                    <TextButton rightImg={<Send2 variant="Bulk" />} text="Share" variant="tertiary" onClick={handleShare} />
                    <TextButton
              ...
              <div className="overflow-y-auto flex flex-col scrollbar-hide border-b border-[#E5E7E3]">
                <section className="space-y-3 py-6 px-8 gap-3 border-b border-[#E5E7E3]">
                  {eventDetails.map((item, index) => (
                    <div key={index} className="flex gap-1 items-center justify-between">
                      ...
                      {item.label === "Location" && (
                        <TagButton
                          rightImg={<Map1 />} text="View on Map" variant="light-purple"
                ...
                {event.chipInDetails && (
                  <div className="space-y-2 py-6 px-8">
                    ...
                    {event.chipInDetails.chipInType === "target" && <ProgressBar current={event.totalDonations} target={event.chipInDetails?.amount} />}
                    {event.chipInDetails.chipInType === "donation" && <ProgressBar variant="minimum-amount" amount={event.chipInDetails?.amount} />}
                    {event.chipInDetails.chipInType === "fixed" && <ProgressBar variant="amount" amount={event.chipInDetails?.amount} />}
                  </div>
                )}
          ...
          <div className="w-full flex-1 overflow-y-auto flex flex-col">
            <main className="flex flex-col place-items-center flex-1">
              ...
              <section className="w-[513px] h-fit flex flex-col gap-6 place-items-center">
                ...
                <img src={event.image} alt={event.title} className="size-[381px] rounded-3xl aspect-square object-cover" />
                <div className="grid gap-2 place-items-center">
                  ...
                  <div className="size-fit flex gap-2 items-center">
                    ...
                    <AvatarGroup size="sm" count={remainingCount} src={goingPhotos} />
                    {event.userResponse === "going" && <AttendanceStatus status="going" />}
                    {event.userResponse === "not-sure" && <AttendanceStatus status="not-sure" />}
                    {event.userResponse === "not-going" && <AttendanceStatus status="not-going" />}
                  </div>
                </div>
                {event.userRole === "host" && (
                  <Alert
                    title="You have manage access to this event" size="sm" option="outline"
